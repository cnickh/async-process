...
;;(defun _create-process (command &key nonblock (encode cffi:*default-foreign-encoding*) directory)
...
(defun create-process (command &key nonblock encode directory)
  (format t "(create-process ~a ~a ~a ~a)" command nonblock encode directory)
  (make-instance 'process :process command :encode encode))
...
(defun delete-process (process)
  (format t "(delete-process ~a)" process))
  
(defun process-pid (process)
  (format t "(process-pid ~a)" process)
  0)
 
(defun process-send-input (process string)
  (format t "(process-send-input ~a ~a)" process string))
  
(defun process-receive-output (process)
  (format t "(process-receive-output ~a)" process)
  nil)
  
(defun process-alive-p (process)
  (format t "(process-alive-p ~a)" process) T)
